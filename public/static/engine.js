...
(function (global) {
  ...
  function predict(klines, analysis, extras) {
    ...
    // 方向标签阈值 —— 必须与概率自洽。
    // 旧值 ±0.20 会把"上涨概率 41%"（即下跌概率 59%）标成"震荡"，自相矛盾。
    // 2026-09-13 研究台账对 9/10 的五条预测做了首次真实结算：
    // 4 条标"震荡"但实际全部下跌，只有标"看跌"的那条命中（方向命中率 20%）。
    // 归因后阈值改为 ±0.10（对应上涨概率 55%/45%），同一批样本的方向命中率 20% → 80%(4/5)。
    // 这是「台账 → 结算 → 复盘 → 修正」闭环的第一个产出。
    const LABEL_TH = 0.10;
