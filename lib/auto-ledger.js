...
/**
 * 当前监控池 —— 统一走 lib/pool.js（唯一事实来源）。
 * 以前这里自己读 notify.config.json 并内置一份 13 只的兜底池，
 * 结果池子加到 16 只后兜底名单没跟上（公网部署读不到 notify.config.json 时就会用到）。
 */
function poolCodes() {
  return Pool.loadPool().map((x) => ({ code: x.code, name: x.name || x.code }));
}
...
/**
 * 末根K线是否是「已收盘的完整日线」。
 * 为什么需要：自动入账用 anchorDate/anchorPrice 作为结算基准，
 * 若在盘中运行，锚定价是**半天的盘中价**而不是收盘价，
 * 到期比对该锚点就会系统性偏移 —— 台账最怕的就是这种静默污染。
 * 判定：末根K线不是今天 → 必然已收盘；是今天 → 需过了 15:05。
 */
function isClosedBar(klines) {
  const last = klines[klines.length - 1];
  if (!last) return false;
  if (last.date !== todayStr()) return true;
  const n = new Date();
  return (n.getHours() * 60 + n.getMinutes()) >= (15 * 60 + 5);
}

/**
 * 批量自动入账
 * @param {Object} opts { codes, date, limit, dryRun, allowIntraday }
 */
async function autolog(opts) {
  ...
  let added = 0, skipped = 0, failed = 0, intraday = 0;
  const results = [];
  for (const { code, name } of list) {
    try {
      ...
      // 盘中不锚定：避免用半日盘中价当结算基准（收盘后由盯盘钩子正常入账）
      if (!opts.allowIntraday && !isClosedBar(klines)) { intraday++; continue; }
  ...
  return { added, skipped, failed, intraday, total: list.length, results, dryRun: !!opts.dryRun };
}
