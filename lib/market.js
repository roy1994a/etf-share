...
async function fetchTencentKlineQfq(tc, p, limit) {
  ...
  try {
    ...
    healthRecord('kline:tencent-qfq', false, Date.now() - t0, e.message, '前复权（首选）');
    throw e;
  }
}
...
async function fetchTencentKlineNoAdj(tc, p, limit) {
  ...
  try {
    ...
    healthRecord('kline:tencent-noadj', false, Date.now() - t0, e.message, '不复权（备用1）');
    throw e;
  }
}
...
async function fetchSinaKline(code, limit) {
  ...
  try {
    ...
    healthRecord('kline:sina', false, Date.now() - t0, e.message, '不复权（备用2，T-1）');
    throw e;
  }
}
...
async function fetchTencentKline(period, limit, code) {
  ...
  } catch (e) { errs.push('qfq:' + e.message); }
  try {
  ...
  } catch (e) { errs.push('noadj:' + e.message); }
  try {
...
function wrapHealth(name, fn) {
  return async function wrapped() {
    ...
    try {
      ...
      healthRecord(name, false, Date.now() - t0, e.message);
      throw e;
    }
  };
}
