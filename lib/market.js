...
async function fetchGlobalHistory(opts) {
  ...
  const [idx, sox, tnx, spx] = await Promise.allSettled([
  ...
  const v = {
    ...
    soxSeries: sox.status === 'fulfilled' ? sox.value : [],
    us10ySeries: tnx.status === 'fulfilled' ? tnx.value : [],
    spxSeries: spx.status === 'fulfilled' ? spx.value : [],
...
module.exports = { httpGet, httpPostJson, httpPostForm, fetchTencentKline, fetchMinute, parseTencentQuote, calibrateKlines, fetchFundFlow, fetchMarketBreadth, fetchNewsSentiment, fetchIndexKline, fetchIndexKlineTencent, fetchIndexKlineAuto, INDEX_TENCENT_TC, fetchYahooHistory, fetchGlobalHistory, fetchHhxgSnapshot, fetchUs10y, fetchCn10y, fetchSox, fetchFundNav, fetchChemFutures, estimatePremium, tencentCode, CODE, SZ_CODE, NAME };
