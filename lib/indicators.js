...
function OBV(klines) {
  ...
  let acc = 0;
  for (let i = 1; i < klines.length; i++) {
    const c = klines[i].close, pc = klines[i - 1].close;
    const v = klines[i].volume || 0;
    if (c > pc) acc += v; else if (c < pc) acc -= v;
    out[i] = acc;
  }
  return out;
}

/** OBV 的 N 日斜率（归一化到 [-1,1]）：用 OBV 变化量 / 期间总成交量 */
function obvSlope(klines, obv, n) {
  const out = new Array(klines.length).fill(null);
  for (let i = n; i < klines.length; i++) {
    const d = obv[i] - obv[i - n];
    let vol = 0;
    for (let t = i - n + 1; t <= i; t++) vol += klines[t].volume || 0;
    out[i] = vol > 0 ? d / vol : 0;
  }
  return out;
}
...
function computeAll(klines) {
  ...
  return {
    closes, volumes, obv,
    obvSlope10: obvSlope(klines, obv, 10),
...
module.exports = { clamp, num, SMA, EMA, RSI, MACD, KDJ, ATR, MFI, BOLL, OBV, obvSlope, computeAll };
