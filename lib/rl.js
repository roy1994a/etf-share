...
function initState() {
  ...
  return {
    ...
    tradingPolicy: null,  // 实盘交易策略：各周期最优阈值（由含费回测给出）
  };
}
...
function loadState(file) {
  try {
    ...
    if (s.tradingPolicy === undefined) s.tradingPolicy = null;
 ...
 */
function hedgeStep(weights, votes, y, eta, gamma, floor, lossType) {
...
function update(state, horizon, votes, y, regime) {
  ...
  // 1) 全局权重
  H.weights = hedgeStep(H.weights, votes, y, eta, gamma, floor, lossType);
  H.n++;
  ...
  if (regime) {
    ...
    R.weights = hedgeStep(R.weights, votes, y, eta, gamma, floor, lossType);
    R.n++;
  }
