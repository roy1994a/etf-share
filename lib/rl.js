...
/**
 * 先验权重 = 现有手工调参的模型（public/static/engine.js 中的 w）。
 * 设计要点：不从零学起，而是**从你已经验证过的手工模型出发**，
 * 让数据去修正它。没被手工选中的专家给一个 0.01 的探索底仓，
 * 一旦它在某个周期上被证明有效，指数更新会自动把权重涨上来。
 */
const PRIOR_WEIGHTS = {
  d1: { mom: 0.30, rsiRev: 0.25, volS: 0.15, soxS: 0.10, senti: 0.10, indexS: 0.10, trend: 0.01, macdDir: 0.01, rel: 0.01, fund: 0.01, macroS: 0.01 },
  d3: { mom: 0.25, trend: 0.20, macdDir: 0.15, volS: 0.15, soxS: 0.10, indexS: 0.10, rel: 0.05, rsiRev: 0.01, senti: 0.01, fund: 0.01, macroS: 0.01 },
  w1: { trend: 0.25, mom: 0.20, fund: 0.15, macroS: 0.15, indexS: 0.10, rel: 0.10, senti: 0.05, rsiRev: 0.01, macdDir: 0.01, volS: 0.01, soxS: 0.01 },
  m1: { trend: 0.25, macroS: 0.20, fund: 0.20, indexS: 0.15, rel: 0.10, mom: 0.10, rsiRev: 0.01, macdDir: 0.01, volS: 0.01, soxS: 0.01, senti: 0.01 },
};
...
function initHorizon(prior) {
  ...
  for (const k of ALL_EXPERTS) weights[k] /= z;
  return { weights, prior: Object.assign({}, weights), stats, n: 0 };
}

function initState() {
  const horizons = {};
  for (const h of HORIZONS) horizons[h] = initHorizon(PRIOR_WEIGHTS[h]);
  return {
    version: 1,
    eta: DEFAULT_ETA,
...
function effectiveWeights(state, horizon, regime) {
  ...
  if (!r || !r.n) return Object.assign({}, g);
  ...
  for (const k of ALL_EXPERTS) out[k] = z ? out[k] / z : 1 / ALL_EXPERTS.length;
  return out;
}
 ...
 */
function hedgeStep(weights, votes, y, eta, gamma, floor) {
  const active = ALL_EXPERTS.filter((k) => votes[k] != null);
  let activeOld = 0;
  ...
  for (const k of active) {
    ...
    const loss = (p - y) * (p - y);        // Brier 损失 ∈ [0,1]
...
function update(state, horizon, votes, y, regime) {
  ...
  for (const k of ALL_EXPERTS) {
    ...
    const loss = (p - y) * (p - y);
    const st = H.stats[k];
    st.n++; st.lossSum += loss; st.brierSum += loss;
    st.hits += ((p >= 0.5 ? 1 : 0) === y) ? 1 : 0;
    if (v === 0) st.abstain = (st.abstain || 0) + 1;   // 弃权（输出 50%）
  }

  // 1) 全局权重
  H.weights = hedgeStep(H.weights, votes, y, eta, gamma, floor);
  H.n++;
  ...
  if (regime) {
    ...
    R.weights = hedgeStep(R.weights, votes, y, eta, gamma, floor);
    R.n++;
  }
...
module.exports = {
  HORIZONS, HORIZON_DAYS, HORIZON_LABEL, PRIOR_WEIGHTS, ALL_EXPERTS,
  ...
  regimeOf, effectiveWeights, aggregate, update,
