...
function stats(db) {
  ...
  // 按预测来源（模型 vs 人工覆写）—— 直接回答"我的直觉比模型准吗"
  const bySource = {};
  for (const o of allOutcomes) {
    const src = o.source || 'model';
    bySource[src] = bySource[src] || { n: 0, hits: 0, brierSum: 0, sumRet: 0 };
    bySource[src].n++;
    bySource[src].hits += o.dirHit ? 1 : 0;
    bySource[src].brierSum += o.brier;
    if (o.actualPct != null) bySource[src].sumRet += (o.predictedDir === '看跌' ? -o.actualPct : o.actualPct);
  }
  for (const k of Object.keys(bySource)) {
    const b = bySource[k];
    b.hitRate = +(b.hits / b.n).toFixed(4);
    b.brier = +(b.brierSum / b.n).toFixed(4);
    b.avgSignedRetPct = +(b.sumRet / b.n).toFixed(2);
    delete b.brierSum; delete b.sumRet;
  }

  // ⚠️ 有效样本量：同一到期日的多个标的高度相关，不能当成独立样本。
  // 例如"13 个标的都在 9/11 预测 9/14"实际上只等于 **1 个**市场观测。
  const distinctDates = new Set(allOutcomes.map((o) => o.toDate).filter(Boolean));
  const distinctCodes = new Set(allOutcomes.map((o) => o.code).filter(Boolean));
  const perDate = {};
  for (const o of allOutcomes) if (o.toDate) perDate[o.toDate] = (perDate[o.toDate] || 0) + 1;
  const maxSameDate = Object.keys(perDate).reduce((m, k) => Math.max(m, perDate[k]), 0);
  const effectiveN = distinctDates.size;
  const correlationWarning = (allOutcomes.length > 0 && effectiveN > 0 && allOutcomes.length / effectiveN >= 3)
    ? `⚠️ ${allOutcomes.length} 个预测只来自 ${effectiveN} 个不同的到期日（单日最多 ${maxSameDate} 个），` +
      `同一天的多个标的高度相关 —— **有效样本量约 ${effectiveN}，不是 ${allOutcomes.length}**。` +
      `按有效样本量看，现在仍远不足以判断准确率。`
    : null;
...
function buildLedgerReport(db, opts) {
  ...
  L.push(`- 已判定预测：**${st.scoredOutcomes}** 个（来自 **${st.distinctDates}** 个不同到期日、${st.distinctCodes} 个标的）`);
  if (st.correlationWarning) { L.push(''); L.push('> ' + st.correlationWarning); }
